...
## Дымовое тестирование ввода объектов на основании

Данная обработка может быть использована и в bdd и в tdd/xdd.
Запускать данный набора тестов рекомендуется базе данных в которой уже есть заполненные объекты.

### Дымовое тестирование xdd

Для заполнения списка исключений объектов из проверки их необходимо заполнить в модуле формы обработки в процедуре `ПолучитьСписокИсключений`

Дымовое тестирование ввода объектов на основании может быть сконфигурировано через файл `smoke.json`, по аналогии с обычными дымовыми тестами. Для этого конфигурационный файл должен содержать объект с ключом `smokeInputBasedOn`, внутри которого задаются исключаемые из тестирования комбинации объектов и их оснований ввода следующим образом:
...
```json
{
    "smokeInputBasedOn": {
        "Исключения": {
            ...
            "Справочники": [
                "ЧтоОткрываем/СправочникОснование"
            ]
        }
    }
}
